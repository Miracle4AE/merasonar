# RC6 — MeraSonar release screenshot + smoke capture (Windows rebuilt exe)
param(
...
# Sidebar Y centers @ 1280x720 (approx)
$sidebarX = 120
Save-Screen "dashboard-online.png" { param($h) }
Save-Screen "dashboard-offline.png" { param($h) } -DismissModal
Save-Screen "dashboard.png" { param($h) }

Save-Screen "live-area.png" { param($h) [MeraSonarWin]::Click($h, $sidebarX, 200) }
Save-Screen "marine-intelligence-screen.png" { param($h) [MeraSonarWin]::Click($h, $sidebarX, 248) }
Save-Screen "map-world.png" { param($h) [MeraSonarWin]::Click($h, $sidebarX, 296) }
Save-Screen "compare-screen.png" { param($h) [MeraSonarWin]::Click($h, $sidebarX, 440) }

Save-Screen "chart-overlay-screen.png" { param($h)
  [MeraSonarWin]::Click($h, $sidebarX, 296)
  Start-Sleep -Seconds 3
  [MeraSonarWin]::Click($h, 850, 72)
}

# Captain Atlas Command Center — sidebar bottom card CTA
Save-Screen "captain-atlas.png" { param($h)
  [MeraSonarWin]::Click($h, 115, 668)
}

# Battery saver — Ayarlar then third performance chip (~Pil Tasarrufu)
Save-Screen "battery-saver-dashboard.png" { param($h)
  [MeraSonarWin]::Click($h, $sidebarX, 536)
  Start-Sleep -Seconds 2
  [MeraSonarWin]::Click($h, 680, 280)
  Start-Sleep -Seconds 1
  [MeraSonarWin]::Click($h, $sidebarX, 152)
}

Write-Host "RC6 screenshot capture complete."
